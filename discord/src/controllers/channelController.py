from services.channelService import (
    ...
    approve_join_request, get_join_requests, reject_join_request, send_message_p2p
)
...
def get_hosted_channels_controller(data):
    username = data.get("username")
    if not username:
         return {"status": "error", "message": "Missing parameters"}
    hosted_result = get_hosted_channels(username)
    if hosted_result["status"] == "error":
        return {"status": "error", "message": "User not found"}
    return {
        "status": "success",
        "data": {
...
def send_message_p2p_controller(data):
    username = data.get("username")
    channel_name = data.get("channel_name")
    message_text = data.get("message")
    if not username or not channel_name or not message_text:
        return {"status": "error", "message": "Missing parameters"}
    result = send_message_p2p(username, channel_name, message_text)
    return result
