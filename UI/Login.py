...
#         # Xử lý logic đăng nhập, mở trang homechat
...
#         # Mở trang homechat trong chế độ chỉ xem
...
class RegisterDialog(QDialog):
    ...
    def validate_registration(self):
        ...
        if not username or not password:
            ...
            self.accept()  # Đóng hộp thoại sau khi đăng ký thành công
...
class LoginRegisterUI(QWidget):
    ...
    def open_register_dialog(self):
        dialog = RegisterDialog()
        if dialog.exec():  # Nếu người dùng nhấn "Register" và thành công
            ...
            # Không mở chatbox ngay lập tức, chỉ hiển thị thông báo
    ...
    def login(self):
        ...
        if not username or not password:
            ...
            return  # **Chặn hoàn toàn việc đăng nhập nếu không nhập thông tin**
        
        print(f"Logged in as: {username}")
        QMessageBox.information(self, "Login Successful", "You have successfully logged in!")
        # Xử lý logic đăng nhập, mở trang homechat nếu cần

    def login_as_viewer(self):
        QMessageBox.information(self, "Viewer Mode", "You are now in viewer mode. You cannot interact with the chat.")
        print("Logged in as Viewer")
        # Mở trang homechat trong chế độ chỉ xem nếu cần
